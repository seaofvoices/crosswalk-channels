return {
    CounterContainerName = '_ReplicatedValuesCounters%%%',
    LocalPlayerContainerName = '___ReplicatedValues__**',
    ListLengthAttribute = '__ReplicatedValues__len_',
    ValueAttribute = '____value_',
    EventName = if _G.DEV
        then '__channels__event__'
        else '8BUiR0ZTgP3%+wI)%wmRMEwf6XyOijZLxhbAT*R*',
    FastEventName = if _G.DEV
        then '__channels__event___'
        else '7Ug$X 1sxcJc7xF@AM(0Kssqqj3Upr*uv1lgw f6',
}
